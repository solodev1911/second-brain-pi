...
def parse_memory_doc(text: str) -> dict[str, Any] | None:
    ...
    Returns the recognised fields (``type``, ``date``, ``question``, ``summary``,
    ``outcome``, ``correction``, ``source_nodes``). ``source_nodes`` is always a list.
    """
    ...
    for line in lines[1:]:
        ...
        if m:
            ...
            if key in ("type", "date", "question", "summary", "outcome", "correction", "contributor"):
                fields[key] = val
    return fields
