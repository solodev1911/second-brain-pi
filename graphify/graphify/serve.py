...
# Modified from Graphify v0.9.64 for Second Brain graph refresh support.
...
async def _tool_refresh_graph(project_path) -> str:
    """Rebuild one project and report the graph produced by the CLI.

    Run from the trusted Graphify package root, never the indexed project.  A
    project may itself contain a top-level ``graphify`` package which would
    otherwise shadow this engine in the child interpreter.
    """
    if not project_path:
        return json.dumps({"ok": False, "error": "project_path is required"})
    root = Path(project_path).resolve()
    if not root.is_dir():
        return json.dumps({"ok": False, "error": f"project_path is not a directory: {root}"})
    env = dict(os.environ)
    ...
    try:
        ...
        return json.dumps({"ok": False, "error": f"could not start graphify update: {exc}"})
    try:
    ...
    if process.returncode != 0:
        ...
        return json.dumps({"ok": False, "error": detail[-4000:] or f"graphify update exited {process.returncode}"})
    try:
        from graphify.memory_links import project_memory_stats
        result = project_memory_stats(root)
    except Exception as exc:
        return json.dumps({"ok": False, "error": f"refresh verification failed: {exc}"})
...
def _build_server(graph_path: str):
    ...
    async def list_tools() -> list[types.Tool]:
        _tools = [
            ...
            types.Tool(
                name="refresh_graph",
                description="Rebuild the selected project's graph and index explicit Second Brain memory records.",
                inputSchema={"type": "object", "properties": {}},
            ),
            types.Tool(
                name="list_prs",
    ...
    async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
        ...
        if name == "refresh_graph":
            return [types.TextContent(type="text", text=await _tool_refresh_graph(project_path))]
